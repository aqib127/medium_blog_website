  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const profileRes = await apiClient(endpoints.userUpdate(user.handle), {
        method: 'PUT',
        body: JSON.stringify({ name, bio, location, twitter, github, website, avatar_color: avatarColor }),
      });
      ...
      <form onSubmit={handleSubmit} className="settings-form">
        <div className="settings-field settings-avatar-field">
          ...
          <div className="avatar-upload-row">
            <Avatar name={name} avatar={avatarPreview} color={avatarColor} size={80} />
            <div className="avatar-upload-buttons">
              <button type="button" className="btn btn-ghost" onClick={() => fileInputRef.current.click()}>Upload</button>
              <input type="file" ref={fileInputRef} accept="image/*" onChange={handleAvatarChange} style={{ display: 'none' }} />
              {(avatar || avatarPreview) && (
                <button type="button" className="btn btn-ghost" onClick={handleRemoveAvatar}>Remove</button>
              )}
            </div>
          </div>
        ...
        <div className="settings-field">
          ...
          <p className="settings-hint">Used when you don't have a profile picture.</p>
        </div>
        ...
        <button type="submit" className="btn btn-primary settings-save" disabled={loading}>
